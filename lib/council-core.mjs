...
// (context-bleed from a prior session), not a member with a particular writing style or
// verbosity. So it is measured on words, via two independent views that must BOTH fail
// before a verdict is dropped:
//   - coverage (precision): fraction of the verdict's vocabulary that is proposal vocabulary.
//     Low on its own is not enough — a thorough verdict introduces lots of fresh analysis
//     words, so a long, on-topic verdict (e.g. dense coined compounds like "state-mediated")
//     dilutes coverage without being off-topic.
//   - anchors (recall): how many of the proposal's own terms the verdict actually echoes.
//     A genuine verdict references the thing it judges; context-bleed echoes ~none.
// We split on non-letters so a hyphenated compound contributes its component words instead
// of one token that never matches. A verdict is flagged only when it is BOTH built from
// foreign vocabulary AND fails to reference the proposal — the signature of bleed, not of a
// writing style. (The old gate flagged on >=8 "foreign" hyphenated terms OR coverage alone;
// both were verbosity/style proxies that systematically false-flagged dense, thorough prose.)
...
export function _anchors(txt, propVocab) {
  let n = 0
